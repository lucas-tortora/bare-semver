...
export class Comparator {
   ...
   * @returns `true` if `version` satisfies the comparator's operator and version, `false` otherwise.
   */
...
export class Range {
  /**
   * @param comparators - Two-dimensional array of comparator sets: the outer array is a union (OR) of inner arrays, each an intersection (AND); defaults to an empty range that matches nothing.
   */
...
export namespace Range {
   ...
   * @throws {INVALID_VERSION} `input` is not valid range syntax (reported via the `INVALID_VERSION` code — `INVALID_RANGE` is not currently thrown by the parser).
   */
